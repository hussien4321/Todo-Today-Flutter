...
**Key libraries** · `sqflite` · `flutter_local_notifications` · `charts_flutter` · `firebase_admob` · `flutter_inapp_purchase` · `path_provider` · `share` · `url_launcher` · `font_awesome_flutter`
...
|---|---|---|
| **A deadline that's right after the app was closed for a week** | Expiry is *derived* from stored timestamps, never ticked by a timer. The app can be killed, backgrounded or offline for days and still open showing the truth. | [↓](#1-deadlines-are-derived-not-ticked) |
| **A countdown on every row** | A live 24-hour clock per task, rendered without a timer per row rebuilding the list. | [↓](#2-one-animation-many-countdowns) |
| **History from an append-only log** | Tasks are templates; todos are attempts. Keeping them separate is what makes "am I improving?" answerable at all. | [↓](#3-tasks-are-templates-todos-are-attempts) |
| **Nudges that survive a cold start** | Per-task deadline notifications and a re-engagement reminder, all scheduled with the OS rather than requiring the app to be running. | [↓](#4-notifications-scheduled-with-the-os) |
...
### 1. Deadlines are derived, not ticked
...
### 2. One animation, many countdowns
...
### 3. Tasks are templates, todos are attempts
...
### 4. Notifications scheduled with the OS
...
- **String-concatenated SQL.** Inserts and updates build statements by concatenation rather than binding parameters. Inputs are local and user-authored so it isn't an injection surface in practice, but it's the wrong default and an apostrophe in a task name is a real bug.
- **Schema v1 with no migration path.** `onCreate` only; there's no upgrade ladder, so a schema change would have meant losing user history.
- **Singletons initialised in a constructor body.** `DBHelper` and `NotificationService` kick off async work from `_internal()` and rely on a later null check. It works, but the first caller can race the initialisation.
- **No tests.** The date arithmetic — bucketing, forfeits, the 24-hour boundary — is the part most worth testing and the part with none.
